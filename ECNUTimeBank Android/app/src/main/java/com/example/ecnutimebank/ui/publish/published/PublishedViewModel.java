...
public class PublishedViewModel extends ViewModel {
    ...
    private PublishedViewModel.OnRequestDoneListener onRequestDoneListener;
...
//        todo 加载时间有点长 切出去之后会直接销毁 希望能保留下来 要么不释放内存 要么保存状态下次恢复
                ...
                .execute(new JsonCallBack<Result<List<Order>>>() {
                    ...
                    public void onSuccess(Response<Result<List<Order>>> response) {
                        if (response.body().getCode() == ResultCode.SUCCESS.getCode()) {
                            Log.i("PublishedViewModel", "success to load 10 more published requirements of user " + userId);
                            List<Order> list = requirementsList.getValue();
                            list.addAll(response.body().getData());
                            requirementsList.postValue(list);
                        } else {
                            Log.e("PublishedViewModel", "fail to load 10 more requirements");
                        }
    ...
    public void setOnRequestDoneListener(PublishedViewModel.OnRequestDoneListener onRequestDoneListener) {
